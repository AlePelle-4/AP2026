...
int main() {
    ...
    if ( leggi_netlist(nomefile,Nome_res,resistenze,generatori,grafo) ) {
        ...
        Eigen::VectorXd correnti=gradiente_coniugato((B.transpose())*R*B,v,x0,tol,it_max);
        //de Pina mi trova i cicli minimi, pertanto nelle colonne di B tenderanno a esserci piu zeri delle
        //colonne di B relative ai cicli fondamentali. Il condizionamento forse è un po meglio con de Pina (matrice piu sparsa)
